...
func CreateTransaction(ctx context.Context, srcAcc, destAcc *model.Account, fund decimal.Decimal) (uuid.UUID, error) {
	id := uuid.New()
	//add create time
	sql := `INSERT INTO t_transaction(uuid, source_account_id, source_init, source_after, destination_account_id, destination_init, destination_after, amount) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
...
func UpdateTransactionSource(ctx context.Context, transId uuid.UUID, srcAcc *model.Account) error {
	sql := `UPDATE t_transaction SET source_after = $1 WHERE uuid = $2`
